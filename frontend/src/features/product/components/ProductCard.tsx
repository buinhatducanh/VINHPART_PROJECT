    ...
    >
      ...
      <div className="relative aspect-square bg-gradient-to-br from-gray-800 to-gray-900 overflow-hidden">
        ...
        {product.discount_percentage && (
          <div className="absolute top-3 left-3 z-10">
            ...
            >
              ...
              <div className="relative bg-gradient-to-br from-yellow-400 to-yellow-600 text-white px-3 py-1.5 rounded-full font-black text-sm shadow-xl border-2 border-yellow-300">
                -{product.discount_percentage}%
              </div>
            </motion.div>
          </div>
        ...
        <div className={`absolute top-3 z-10 ${product.discount_percentage ? 'right-3' : 'right-3'}`}>
          {product.stock_status === 'in_stock' && (
            <motion.span
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className="px-3 py-1.5 bg-gradient-to-r from-green-600 to-green-500 text-white text-xs font-bold rounded-full shadow-lg shadow-green-600/50"
            >
              Còn hàng
            </motion.span>
          )}
          {product.stock_status === 'low_stock' && (
            <motion.span
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className="px-3 py-1.5 bg-gradient-to-r from-yellow-600 to-yellow-500 text-white text-xs font-bold rounded-full shadow-lg shadow-yellow-600/50"
            >
              Sắp hết
            </motion.span>
          )}
          {product.stock_status === 'out_of_stock' && (
            <motion.span
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className="px-3 py-1.5 bg-gradient-to-r from-gray-600 to-gray-500 text-white text-xs font-bold rounded-full shadow-lg"
            >
              Hết hàng
            </motion.span>
          )}
        ...
        <div className="absolute bottom-3 left-3 z-10">
          <span className="px-3 py-1.5 bg-black/80 backdrop-blur-md text-white text-xs font-bold rounded-full border border-red-600/30 shadow-lg">
            {product.vehicle_type === 'Motorbike' ? '🏍️ Xe máy' : '🚗 Ô tô'}
          </span>
        </div>
      </div>
      ...
      <div className="p-4 relative z-10">
        <h3 className="text-white font-bold mb-2 line-clamp-2 min-h-[3rem] group-hover:text-red-400 transition-colors">
        ...
        <div className="flex gap-2">
          <motion.button
            ...
            className="flex-1 flex items-center justify-center gap-1.5 px-3 sm:px-4 py-2.5 sm:py-3 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 disabled:from-gray-700 disabled:to-gray-700 disabled:cursor-not-allowed text-white font-bold rounded-lg transition-all text-xs sm:text-sm shadow-lg shadow-red-600/30 hover:shadow-red-600/50 whitespace-nowrap touch-manipulation"
          >
          ...
          >
            ...
            <span className="hidden sm:inline truncate text-xs sm:text-sm">Thêm</span>
          </motion.button>
        </div>
      </div>
    </motion.div>
