...
import { ManageSEOPage } from '@/components/admin/ManageSEOPage';
import { ManageCategoriesPage } from '@/components/admin/ManageCategoriesPage';
import { ManageReviewsPage } from '@/components/admin/ManageReviewsPage';

interface AdminDashboardProps {
  onBackToHome: () => void;
}

type AdminPage = 'dashboard' | 'addProduct' | 'manageProducts' | 'manageOrders' | 'statistics' | 'settings' | 'manageSEO' | 'manageCategories' | 'manageReviews';
...
export function AdminDashboard({ onBackToHome }: AdminDashboardProps) {
  ...
  const quickActions = [
    { label: 'Thêm sản phẩm mới', icon: Plus, color: 'text-blue-400', hoverColor: 'hover:bg-blue-500/10', onClick: () => setCurrentPage('addProduct') },
    { label: 'Xem đơn hàng mới', icon: ClipboardList, color: 'text-purple-400', hoverColor: 'hover:bg-purple-500/10', onClick: () => setCurrentPage('manageOrders') },
    { label: 'Thống kê doanh thu', icon: TrendingUp, color: 'text-green-400', hoverColor: 'hover:bg-green-500/10', onClick: () => setCurrentPage('statistics') },
    { label: 'Quản lý sản phẩm', icon: Edit, color: 'text-orange-400', hoverColor: 'hover:bg-orange-500/10', onClick: () => setCurrentPage('manageProducts') },
    { label: 'Quản lý bài viết SEO', icon: FileText, color: 'text-purple-400', hoverColor: 'hover:bg-purple-500/10', onClick: () => setCurrentPage('manageSEO') },
    { label: 'Quản lý danh mục', icon: Folder, color: 'text-yellow-400', hoverColor: 'hover:bg-yellow-500/10', onClick: () => setCurrentPage('manageCategories') },
    { label: 'Quản lý đánh giá', icon: MessageSquare, color: 'text-cyan-400', hoverColor: 'hover:bg-cyan-500/10', onClick: () => setCurrentPage('manageReviews') },
  ];
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {quickActions.map((action, index) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  onClick={action.label === 'Xem trang chủ' ? onBackToHome : action.onClick}
