...
export function AddProductPage({ onBack }: AddProductPageProps) {
      ...
      widgetRef.current = (window as any).cloudinary.createUploadWidget(
        {
          ...
          uploadPreset: 'vinhpart_products_preset', // Or use unsigned, or signed. 
          // Plan said signed. So we need to fetch signature.
          // Note: Widget support signing by providing a function.
          // Let's use the simple unsigned for now IF the user enabled it, OR signed.
          // The plan said: "GET /signature: Endpoint to generate a signature".
          // So let's implement signed upload.
          apiKey: (import.meta as any).env.VITE_CLOUDINARY_API_KEY || '351913623539441',
          uploadSignature: generateSignature,
          uploadSignatureTimestamp: Date.now() // timestamp is generated by function? No.
          // Actually, 'uploadSignature' expects a function calling our backend.
        },
        (error: any, result: any) => {
          if (!error && result && result.event === "success") {
            const url = result.info.secure_url;
            // Add f_auto,q_auto if not present (usually widget does it if configured, or we append)
            // Let's just use the secure_url
  ...
  const generateSignature = async (callback: any, params_to_sign: any) => {
    try {
      const response = await fetch(`http://localhost:3001/api/upload/signature?timestamp=${params_to_sign.timestamp || Date.now() / 1000}`);
  ...
  const openWidget = () => {
    if (widgetRef.current) {
      // We need to pass signature parameters dynamically maybe?
      // The generateSignature callback handles it.
      // Wait, for signed uploads we need to fetch signature matching the params.
      // Simplified: Use a standard call.
      // Actually, the widget handles the flow. We just provide the signing function.
      // But the signature function needs to return the signature for the specific timestamp/params.
      // My backend endpoint generates a FRESH timestamp. 
      // The widget provides params_to_sign which includes timestamp.
      // My backend MUST sign the params provided by the widget.
      // Current backend `generateSignature` ignores params and makes its own timestamp.
      // CHECK backend/lib/cloudinary.ts: 
      // const timestamp = Math.round((new Date).getTime() / 1000);
      // It generates its own.
      // This is fine for *direct* api calls but for Widget, the WIDGET generates the timestamp usually?
      // Let's re-read Cloudinary Widget docs mentally.
      // Widget option: `uploadSignature` is a function (callback, params_to_sign).
      // We must sign `params_to_sign`.
      // My backend signature logic is hardcoded to `folder: 'vinhpart_products'`.
      // If widget sends other params, signature fails.
      // I need to update backend to sign REQUESTED params.

      // PAUSE. Updating backend first to support signing arbitrary params (or specific ones).
      // Let's adjust the plan. I will assume unsigned for a moment? No, user provided credentials imply security.
      // I'll update backend signature endpoint to accept params to sign.
